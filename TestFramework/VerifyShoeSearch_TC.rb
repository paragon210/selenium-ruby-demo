# This class is a test case to test the search term "shoes" on eBay
...
class VerifyShoeSearch < Test::Unit::TestCase
  def setup
  ...
  def test_verify_shoe_search
    ...
    @ebayPage.submit_correct_search(@driver, @wait)

    #Verify we are on a page of search results using our search term
    assert_equal(@driver.title,"#{@ebayPage.search_term} | eBay")

    #Verify the search term is the term on the page
    assert_true(@resultsPage.verify_search_term(@driver, "#{@ebayPage.search_term}"))
